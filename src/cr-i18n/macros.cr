...
macro label(target, locale = "", count = nil, **splat)
  {% if flag?(:enforce_labels) || flag?(:enforce_label_parity)
  ...
  CrI18n.get_label({{target.is_a?(StringInterpolation) ? target : "#{target.id}"}}, {{locale}}, count: {{count}}, {% for name, val in splat %}{{name.id}}: {{val}},{% end %})
end
